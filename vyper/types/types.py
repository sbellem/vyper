...
class TupleLike(NodeType):
    ...
    def tuple_items(self):
        raise NotImplementedError(
            "compiler panic!: tuple_items must be implemented by TupleLike"
        )
...
class StructType(TupleLike):
    ...
    def __repr__(self):
        ...
        return (
            prefix
            + "{"
            + ", ".join([k + ": " + repr(v) for k, v in self.members.items()])
            + "}"
        )
...
def make_struct_type(name, location, members, custom_structs, constants):
    ...
    for key, value in members:
        ...
        o[key.id] = parse_type(
            value, location, custom_structs=custom_structs, constants=constants,
        )
...
def parse_type(item, location, sigs=None, custom_structs=None, constants=None):
    ...
    if isinstance(item, vy_ast.Name):
        ...
        # elif item.id in MPC_TYPES:
        #    return MpcType(item.id)
