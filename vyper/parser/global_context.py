...
class GlobalContext:
        ...
        for item in vyper_module:
            ...
            if isinstance(item, vy_ast.ClassDef):
                ...
                if item.class_type == "struct":
                    ...
                    if (
                        item.name in global_ctx._contracts
                        or item.name in global_ctx._interfaces
                    ):
                        raise StructureException(
                            f"Contract '{item.name}' is already defined", item,
                        )
                    global_ctx._contracts[item.name] = GlobalContext.make_contract(item)
                elif item.class_type == "mpc":
                    if item.name in global_ctx._mpcs:
                        raise StructureException(
                            f"MPC '{item.name}' is already defined", item,
                        )
                    global_ctx._mpcs[item.name] = GlobalContext.make_mpc(item)
                else:
                    raise StructureException(
            ...
            # variable_name: type
                ...
                is_implements_statement = (
                    isinstance(item.target, vy_ast.Name)
                    and item.target.id == "implements"
                ) and item.annotation
            ...
            # Function definitions
            # XXX elif isinstance(item, vy_ast.FunctionDef):
            elif isinstance(item, (vy_ast.FunctionDef, vy_ast.AsyncFunctionDef)):
                if item.name in global_ctx._globals:
                    raise FunctionDeclarationException(
                        f"Function name shadowing a variable name: {item.name}"
                    )
                global_ctx._defs.append(item)
            # XXX Async Function definitions
            # elif isinstance(item, vy_ast.AsyncFunctionDef):
            #    if item.name in global_ctx._globals:
            #        raise FunctionDeclarationException(
            #            f"Function name shadowing a variable name: {item.name}"
            #        )
            #    global_ctx._asyncdefs.append(item)
    ...
    def mk_getter(cls, varname, typ):
        ...
        return [
            f"@public\n@constant\ndef {varname}{funname}({head.rstrip(', ')}) -> {base}: return self.{varname}{tail}"  # noqa E501
    ...
    def make_mpc(code):
        ...
        for item in code:
            # Function definitions
            if isinstance(item, (vy_ast.FunctionDef, vy_ast.AsyncFunctionDef)):
                _defs.append(item)
            else:
                raise StructureException("Invalid contract reference", item)
        return _defs
    ...
    def add_constant(self, item):
        ...
        is_valid_struct = (
            len(args) == 1
            and isinstance(args[0], (vy_ast.Subscript, vy_ast.Name, vy_ast.Call))
        ) and item.target
    ...
    def add_globals_and_events(self, item):
        ...
        if not (self.get_call_func_name(item) == "event"):
            ...
            if not all(
                [attr in VALID_GLOBAL_KEYWORDS for attr in item_attributes.keys()]
            ):
                raise StructureException(
        ...
        # Don't move this check higher, as unit parsing has to happen first.
            ...
            if (
                isinstance(item.annotation.args[0], vy_ast.Name)
