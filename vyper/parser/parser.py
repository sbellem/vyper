...
def parse_other_functions(
    ...
    for _def in otherfuncs:
        sub.append(
            parse_function(
                _def, {**{"self": sigs}, **external_contracts}, origcode, global_ctx
            )
        )
        sub[-1].total_gas += add_gas
        add_gas += 30
        for sig in sig_utils.generate_default_arg_sigs(
            _def, external_contracts, global_ctx
        ):
    ...
    if default_function:
        ...
        fallback = LLLnode.from_list(
            ["revert", 0, 0], typ=None, annotation="Default function"
        )
